...
export function useAuth() {
  ...
  const checkSession = useCallback(async () => {
    try {
      const resp = await fetch(`${BFF_BASE}/api/auth/me`, { credentials: 'include' })
      if (resp.ok) {
  ...
  const login = useCallback(() => {
    ...
    const redirectUri = BFF_BASE ? `${BFF_BASE}/auth/callback` : `${window.location.origin}/auth/callback`
  ...
  const claimUsername = useCallback(async (username: string) => {
    const resp = await fetch(`${BFF_BASE}/api/auth/username`, {
      method: 'POST',
